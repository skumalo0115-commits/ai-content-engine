...
export function MobileScrollHint({ label = "Scroll down", className = "" }: MobileScrollHintProps) {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: 10 }}
      transition={{ repeat: Number.POSITIVE_INFINITY, duration: 1.35, ease: "easeInOut" }}
      className={`pointer-events-none fixed inset-x-0 bottom-4 z-30 flex justify-center px-4 ${className}`}
    >
